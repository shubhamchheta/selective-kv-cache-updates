...
def clean_up(kv: DynamicCache, origin_len: int):
    ...
    for i in range(CacheAdapter.num_layers(kv)):

        keys = CacheAdapter.get_keys(kv, i)
        values = CacheAdapter.get_values(kv, i)

        CacheAdapter.set_keys(
            kv,
            i,
            keys[:, :, :origin_len, :].clone()
        )

        CacheAdapter.set_values(
            kv,
            i,
            values[:, :, :origin_len, :].clone()
        )
...
def prepare_kvcache(documents, filepath: str = "./data_cache/cache_knowledges.pt", answer_instruction: str | None = None):
    ...
    print("kvlen: ", CacheAdapter.get_keys(kv, 0).shape[-2])
...
def kvcache_test(args: argparse.Namespace):
    ...
    if args.dataset == "textbook":
    	...
    	if args.mode == "baseline":
        	...
        	print("\nRunning FULL REBUILD experiment\n")
        	text_list, dataset = textbook_dataset.load("v2")

    	elif args.mode == "selective":

        	print("\nRunning SELECTIVE UPDATE experiment\n")
    ...
    kv_len = CacheAdapter.get_keys(knowledge_cache,0).shape[-2]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', required=True, help='Dataset to use (kis, kis_sample, squad-dev, squad-train)',
                        choices=['kis', 'kis_sample',
                                 ...
                                 'hotpotqa-dev',  'hotpotqa-train', 'hotpotqa-test','textbook'])
    parser.add_argument("--mode",choices=["baseline","rebuild","selective"],default="baseline",help="Experiment mode")
